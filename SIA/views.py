...
class CVInvestigadorPDF(View):
    ...
    def get(self, request, pk):
        ...
        capitulos_libros_divulgacion_editoriales_extranjeras = CapituloLibroInvestigacion.objects.filter(autores=pk,
        ...
        libros_docencia = Libro.objects.filter(autores=pk, tipo='DOCENCIA').filter(Q(status='PUBLICADO') & Q(status='EN_PRENSA')).order_by('-fecha')
        ...
        premios_nacionales = DistincionAcademico.objects.filter(usuario=pk, distincion__tipo='PREMIO').filter(distincion__institucion__nombre='México').order_by('-fecha')
        premios_internacionales = DistincionAcademico.objects.filter(usuario=pk, distincion__tipo='PREMIO').exclude(distincion__institucion__nombre='México').order_by('-fecha')
        reconocimientos_nacionales = DistincionAcademico.objects.filter(usuario=pk, distincion__tipo='RECONOCIMIENTO').filter(distincion__institucion__nombre='México').order_by('-fecha')
        reconocimientos_internacionales = DistincionAcademico.objects.filter(usuario=pk, distincion__tipo='RECONOCIMIENTO').exclude(distincion__institucion__nombre='México').order_by('-fecha')
