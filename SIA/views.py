...
class CVInvestigadorPDF(View):
    ...
    def get(self, request, pk):
        ...
        premios_nacionales = DistincionAcademico.objects.filter(usututoresario=pk, distincion__tipo='PREMIO').filter(institucion__nombre='México').order_by('-fecha')
        premios_internacionales = DistincionAcademico.objects.filter(tutores=pk, distincion__tipo='PREMIO').exclude(institucion__nombre='México').order_by('-fecha')
        reconocimientos_nacionales = DistincionAcademico.objects.filter(tutores=pk, distincion__tipo='RECONOCIMIENTO').filter(institucion__nombre='México').order_by('-fecha')
        reconocimientos_internacionales = DistincionAcademico.objects.filter(tutores=pk, distincion__tipo='RECONOCIMIENTO').exclude(institucion__nombre='México').order_by('-fecha')
