...
class CVInvestigadorDetalle(View):
    ...
    def get(self, request, pk):
        ...
        articulos_indexadas_extranjeras = ArticuloCientifico.objects.filter(autores=pk, indices__isnull=False).exclude(revista__pais__nombre='México').filter(Q(status='PUBLICADO') | Q(status='EN_PRENSA')).order_by('-fecha')
