...
class DistincionAcademicoForm(forms.ModelForm):
    distincion = forms.ModelChoiceField(Distincion.objects.all(), widget=wSelect, required=True)
    otorga = forms.ModelChoiceField(Dependencia.objects.all(), widget=wSelect, required=True)
...
class DistincionAlumnoForm(forms.ModelForm):
    distincion = forms.ModelChoiceField(Distincion.objects.all(), widget=wSelect, required=True)
    alumno = forms.ModelChoiceField(User.objects.all(), widget=wSelect, required=True)
