...
from . serializers import EventoDivulgacionSerializer
...
class LibroDivulgacionJSON(View):
    ...
    def get(self, request):
        ...
        try:
            usuarioid = User.objects.get(username=request.user.username).id
            if self.otros:
                items = Libro.objects.filter(tipo='DIVULGACION').exclude(Q(autores__id__exact=usuarioid) & Q(editores__id__exact=usuarioid) & Q(coordinadores__id__exact=usuarioid) & Q(compiladores__id__exact=usuarioid) & Q(agradecimientos__id__exact=usuarioid))
            else:
                items = Libro.objects.filter(tipo='DIVULGACION').filter(Q(autores__id__exact=usuarioid) & Q(editores__id__exact=usuarioid) | Q(coordinadores__id__exact=usuarioid) | Q(compiladores__id__exact=usuarioid) | Q(agradecimientos__id__exact=usuarioid))
            json = serializers.serialize('json', items, use_natural_foreign_keys=True,
...
class RESTEventoDivulgacionLista(generics.ListCreateAPIView):
    queryset = EventoDivulgacion.objects.all()
    serializer_class = EventoDivulgacionSerializer


class RESTEventoDivulgacionDetalle(generics.RetrieveUpdateDestroyAPIView):
    queryset = EventoDivulgacion.objects.all()
    serializer_class = EventoDivulgacionSerializer
...
class EventoDivulgacionDetalle(ObjectModalUpdateMixin, View):
    form_class = EventoDivulgacionForm
    model = EventoDivulgacion
    template_name = 'modal/form_detalle_eventoDivulgacion.html'
