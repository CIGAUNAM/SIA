...
class PublicacionTecnicaJSON(View):
    otros = False
    def get(self, request):
...
class PublicacionTecnicaLista(ObjectCreateVarMixin, View):
    form_class = PublicacionTecnicaForm
    model = PublicacionTecnica
    aux = InformeTecnicoContext.contexto
    template_name = 'publicacion_tecnica.html'


class PublicacionTecnicaDetalle(ObjectUpdateVarMixin, View):
    form_class = PublicacionTecnicaForm
    model = PublicacionTecnica
    aux = InformeTecnicoContext.contexto
    template_name = 'publicacion_tecnica.html'
...
class PublicacionTecnicaEliminar(View):
    def get(self, request, pk):
        try:
...
class LibroInvestigacionJSON(View):
    ...
    def get(self, request):
        ...
        try:
            usuarioid = User.objects.get(username=request.user.username).id
            if self.otros:
                items = Libro.objects.filter(tipo='INVESTIGACION').exclude(Q(autores__id__exact=usuarioid) & Q(editores__id__exact=usuarioid) & Q(coordinadores__id__exact=usuarioid) & Q(compiladores__id__exact=usuarioid) & Q(agradecimientos__id__exact=usuarioid))
            else:
                items = Libro.objects.filter(tipo='INVESTIGACION').filter(Q(autores__id__exact=usuarioid) & Q(editores__id__exact=usuarioid) | Q(coordinadores__id__exact=usuarioid) | Q(compiladores__id__exact=usuarioid) | Q(agradecimientos__id__exact=usuarioid))
            json = serializers.serialize('json', items, use_natural_foreign_keys=True,
