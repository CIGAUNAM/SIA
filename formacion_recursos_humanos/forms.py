...
class AsesorEstanciaForm(forms.ModelForm):
    asesorado = forms.ModelChoiceField(User.objects.all(), widget=wSelectSingle, required=True)
    ...
    programa_licenciatura = forms.ModelChoiceField(ProgramaLicenciatura.objects.all(), widget=wSelectSingle, required=True)
    programa_maestria = forms.ModelChoiceField(ProgramaMaestria.objects.all(), widget=wSelectSingle, required=True)
    programa_doctorado = forms.ModelChoiceField(ProgramaDoctorado.objects.all(), widget=wSelectSingle, required=True)
    beca = forms.ModelChoiceField(Beca.objects.all(), widget=wSelectSingle, required=True)
    proyecto = forms.ModelChoiceField(Proyecto.objects.all(), widget=wSelectSingle, required=True)
    dependencia = forms.ModelChoiceField(Dependencia.objects.all(), widget=wSelectSingle, required=True)
...
class DireccionTesisForm(forms.ModelForm):
    ...
    dependencia = forms.ModelChoiceField(Dependencia.objects.all(), widget=wSelectSingle, required=True)
    beca = forms.ModelChoiceField(Beca.objects.all(), widget=wSelectSingle, required=True)
    reconocimiento = forms.ModelChoiceField(Reconocimiento.objects.all(), widget=wSelectSingle, required=True)
...
class ComiteTutoralForm(forms.ModelForm):
    ...
    asesorado = forms.ModelChoiceField(User.objects.all(), widget=wSelectSingle, required=True)
    asesor_principal = forms.ModelChoiceField(User.objects.all(), widget=wSelectSingle, required=True)
    proyecto = forms.ModelChoiceField(Proyecto.objects.all(), widget=wSelectSingle, required=True)
    dependencia = forms.ModelChoiceField(Dependencia.objects.all(), widget=wSelectSingle, required=True)
